...
Slack 비공개 채널의 같은 스레드, Telegram 봇의 같은 개인 대화, 터미널 세션마다 Codex 대화를 유지합니다. 이름을 외우지 말고 `지하철 앱 로그인 문제 좀 봐줘`처럼 설명하세요. Codex가 Orca의 프로젝트·작업 공간·폴더형 workspace·진행 작업과 터미널 기록을 조회하고 답합니다. 여러 프로젝트를 함께 조회할 수 있으며, 조사해도 대상이 모호할 때 발견한 차이를 설명하며 질문합니다. `GH workspace에서 작업 중일 거야`처럼 추가 설명하면 그 단서로 조사를 이어갑니다. 변경 대상이 확실하지 않으면 코드 작업을 시작하지 않습니다.
...
전역 명령 등록 후에는 어느 폴더에서든 대화를 시작합니다.
...
```sh
hq chat --session my-work
hq ask --session my-work '지하철 앱 작업하고 싶어'
hq ask --session my-work '모바일 화면 쪽이야'
hq ask --session my-work '현재 진행 상황 알려줘'
```

`/new`는 새 대화, `/exit` 또는 입력 종료(EOF)는 터미널 대화 종료입니다. `취소`라고 답하면 대기 중 생성·등록 제안을 해제하고 대화를 이어갑니다. 대화를 초기화해도 이미 실행 중인 Orca 작업을 중지하지는 않습니다. 작업 중지는 별도 작업 명령으로 지시하세요. 서로 다른 채널·스레드·터미널 세션의 선택은 자동으로 합치지 않습니다.

대화에는 기존 Codex 로그인 인증을 사용하며 텍스트 대화를 위해 OpenAI API 키를 추가할 필요가 없습니다. Codex App Server가 영속 대화와 도구 호출을 처리하고, 실제 개발은 Orca native 실행 경로로 전달합니다. `$CODEX_HOME/orca-hq-session-home`(기본 `~/.codex/orca-hq-session-home`)에 HQ 전용 Codex 기록을 보관하며 원래 인증 파일을 링크합니다. HQ의 `conversations.sqlite`에는 채널별 thread ID와 도구 처리 기록을 저장합니다. 재시작 후 같은 대화를 재개합니다.

여러 조회가 필요한 답변에는 시간이 걸릴 수 있습니다. Codex 한 응답의 제한 시간은 180초, 터미널 요청 제한 시간은 240초입니다. 연결 중단 전에 작업이 이미 전달됐을 수 있으므로 상태를 확인하고 결과 불명인 작업을 자동으로 재실행하지 않습니다. `/new`는 대화 맥락을 새로 시작하며 기존 작업은 유지합니다.
