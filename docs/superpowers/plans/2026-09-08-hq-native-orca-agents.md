...
- Execution update: user authorized incremental implementation after the read-only audit. Implement one task at a time with one Orca worker and focused coordinator review. Tasks 1 through 4 are implemented; Task 4's sequential independent review is complete and all five of its findings are fixed, so Task 5 is next; no deployment, service restart, commits, pushes, Slack or Telegram messages in this increment. Earlier planning-only statements below describe the original planning turn.
...
- [x] Replace the `conversation.execute` dependency in a managed-runtime fixture with a throwing spy; submit substantive analysis and assert native Task/Dispatch launch, zero hidden conversation execution, and no completed request before worker result.
- [x] Test `worker.ready` absent before a ready launch receipt and present only with matching native identity. Keep acceptance and launching visible during delayed startup.
- [x] Test one delivery containing success, question and stale completion. Required result: valid task settles once, pending question persists across reconnect, stale completion has no effect, cleanup policy executes before ack.
- [x] Test duplicate delivery and crash after result persistence but before retain/release or ack. Replayed cleanup is idempotent. Uncertain cleanup records exact recovery and never falls back to raw terminal close.
- [x] Test same-context active guidance is durable inbox mail, completed-context followup creates a new Dispatch, and changed scope queues a new item. Never call `terminal send` into a busy or user-owned terminal.
- [x] Test fanout planning uses HQ admission: planner worker settles before waiting for children; child references form a validated DAG, children plus unrelated work cannot exceed the configured finite limit (10 by default); unlimited mode still enforces ownership and resource exclusion.
- [x] Run `pnpm exec vitest run apps/gateway/test/native-coordinator.test.ts apps/gateway/test/managed-runtime.test.ts apps/gateway/test/progress-runtime.test.ts apps/gateway/test/relay-coordinator.test.ts apps/gateway/test/managed-commands.test.ts`; implement callback wiring and delivery journal, rerun.
- [x] Ensure legacy submit/followup/retry and all channel entries use shared admission; preserve permission checks and user/project identity across adapters. Context routing must not silently force all sources to terminal/local.
- [x] With `GATEWAY_EXTERNAL_ADAPTERS` selected, test that the alternate execution service cannot launch a root/verifier/fix/retry outside the admission boundary. Initial migration returns `native_admission_required` for unsupported execution composition rather than launching or silently changing providers; document this explicit compatibility limitation.

Completed in the fourth implementation increment (2026-09-15): 188 expanded focused tests across 11 files, `pnpm typecheck`, and the full suite (91 files / 1191 tests) passed. The managed runtime now uses native admission, durable Delivery/message journals, cleanup-before-ack, channel identity preservation, retained followup, fanout, questions, restart reconciliation, and the external-adapter execution fence. Retain-primary/release-children is an explicit coordinator-selected implementation default, not a previously confirmed user preference. CLI 1.4.199 camelCase receipt compatibility was checked with read-only inspection and isolated transports. Sequential independent review (`../implementation/2026-09-15-native-orca-task4-review.md`) then ran and returned one blocker plus four follow-ups; all five are fixed in a second pass, with resolutions and one reasoned partial disagreement recorded in that review document. Malformed worker content is now durably quarantined per message instead of wedging the delivery loop, restart restores guidance only for a verified live exactly-owned receipt — proven by the authoritative `worker-list projection.liveness` verdict rather than a connected PTY, with a dedicated relay fixture test — declared read-only scope resolves through project aliases and fails closed, the no-conversation-execution acceptance test drives the production composition router, and `HQ_MAX_ACTIVE_WORKERS` is validated with an actionable error. See `../implementation/2026-09-15-native-orca-task4-result.md`. No installation, restart, commit, push, new worktree, nested agent, or live product-worker launch occurred; Tasks 5–7 remain. Historical checkpoint `86e3845` had already passed typecheck, 1165 tests and recursive build; this is distinct from the fresh Task 4 evidence.

## Task 5: Display actual Orca execution in HQ

**Files:**
- Modify: `packages/installer/src/chat.ts`, `packages/installer/src/cli.ts`, `packages/installer/src/progress-client.ts`, `packages/installer/src/watch.ts` and corresponding tests.
- Modify: `packages/core/src/pilot-config.ts`, `packages/installer/src/setup.ts`, `apps/gateway/src/managed-runtime.ts` and their existing test owners.
- Modify: `README.md`.
- Scope adjustment (coordinator-approved, 2026-09-15): the original file list named `apps/gateway/src/config.ts` (`GatewayConfigSchema`), `packages/installer/src/config-files.ts`, `apps/gateway/src/dashboard.ts`, `apps/web/src/routes/command-list.tsx` and `apps/web/src/routes/command-detail.tsx`. The installed `hq` runtime reaches execution through `entry.ts` -> `startManagedRuntime()` -> `PilotConfigSchema` + `managed-runtime.ts` and serves the v1 `/v1/progress` API; the `GatewayConfigSchema` / `dashboard.ts` / `apps/web` ControlStore path is a separate, currently inactive web surface. Native settings were therefore added to `PilotConfigSchema` where they are actually consumed, and `config-files.ts` was handled at its real generator (`setup.ts`). Wiring the legacy web UI to native state is a separate web feature, deliberately not done here to avoid adding unused fields or a new monitoring site.

**Interfaces:** Configuration adds `nativeExecution.maxActiveWorkers` (default 10), `nativeExecution.roleProfiles` and `nativeExecution.retentionPolicy` for completed-primary terminals. Chat and watch consume the Task 1 native events and display requested/effective model plus worktree/terminal/task identity. Native execution is server-owned and independent of every viewer flag.

- [x] Add tests that an unspecified watch preference opens no macOS Terminal window, that an explicit `--progress-window=auto` remains an optional watch viewer, and that `--progress-window=off` never disables native launch. Also fixed the underlying bug: `chat.ts`/`cli.ts` previously opened a window by default whenever the flag was unspecified. Proven on both sides: client-side in `packages/installer/test/chat.test.ts`, and through the real gateway composition in `apps/gateway/test/managed-runtime.test.ts` ("launches native work and records the receipt with no viewer attached at all"), which launches and records `worker.ready` with no window manager, no watch process and no viewer lease.
- [x] Add a receipt-backed display assertion: output includes the exact returned worker handle and effective model; a delayed launch displays waiting rather than running; a missing handle displays a verification state, not an invented reference. `describeNativeWorkerEvent()` renders only the fields the payload actually carries, keeps the reported provider visible when `effective.model` is absent, claims a requested/effective mismatch only on fields the worker actually reported, and sanitizes every interpolated server value.
- [x] Add a PTY regression for partial Korean input during acceptance, queue transition and final response.
- [x] Prove the job-list/help vs project-analysis spawn distinction end to end: `apps/gateway/test/context-router.test.ts` covers the classification, and `apps/gateway/test/managed-runtime.test.ts` ("answers a job-list lookup with zero native launches...") now proves the consequence through the real composition — a Korean job-list request creates zero admission attempts and zero launches, while project work in the same runtime launches exactly one worker.
- [x] Config file -> runtime wiring is proven from the config file's own text: `resolveNativeExecutionSettings()` is what `startManagedRuntime()` uses, and its test feeds it configs produced by `parsePilotConfigText` and asserts admission limit, retention policy and role profiles, plus the one-directional precedence (file wins; `HQ_MAX_ACTIVE_WORKERS` fills only an unset limit). `PilotConfigSchema` rejects null/fractional/zero/negative/non-safe worker limits, unknown retention policies, malformed profiles and a declared role-profile set without `primary`. `hq setup` re-runs now preserve an existing `nativeExecution` block instead of resetting it.
- [x] Document actual native terminal close versus chat/watch close, configured active-worker capacity (default 10) versus retained idle terminals, mixed-provider failures and how to find the Orca terminal. No automatic desktop focus stealing.
- [x] Preserve source-tagged output, `observedAt` separately from native `updatedAt`, and unknown/release states in projections — for the surfaces the installed runtime actually serves (`watch.ts` and `chat.ts`). The legacy `dashboard.ts` / `apps/web` projections are out of scope per the adjustment above and remain on the legacy job model.
- [x] Full suite green with no unhandled rejections: the pre-existing `progress_runtime_closed` unhandled rejection in `apps/gateway/test/managed-runtime.test.ts` was caused by tests abandoning an in-flight `execute()` before `close()`; those promises are now tracked and asserted to fail only for the shutdown reason.

Review record: `docs/superpowers/implementation/2026-09-15-native-orca-task5-review.md`.
...
- [x] Seed the real SQLite fixture with old completed, active, uncertain and queued assignments; reopen under migration. Assert completed/active/uncertain jobs create zero replacement workers and queued untouched requests use the selected new backend exactly once.
- [x] Add real-socket tests for two contexts, same-context followup, mixed-channel shared capacity, 10+1, failure result, stop, unknown launch, retained terminal reacquisition and gateway restart. Fixture Orca transport must be isolated from installed production state.
- [x] Run `pnpm exec vitest run tests/e2e/native-orca-progress.spec.ts tests/e2e/context-progress.spec.ts apps/gateway/test/progress-store.test.ts apps/gateway/test/restart-fixture.test.ts apps/gateway/test/managed-service.test.ts` and implement migration/recovery fixes.
- [x] Run `pnpm test`, `pnpm typecheck`, and a safe all-workspace build once integration is stable. The verified build command was `pnpm --filter './packages/**' --filter './apps/**' --if-present run build`, avoiding the root global-installer `prepare`; fresh result: 93 files / 1231 tests.
- [x] Write exact changed-file allowlist and installation/rollback sequence: backup source/build/config/DB; preserve current active work; install matched build artifacts; restart normally; reconcile; check health and new request. A rollback stops new admissions, preserves current DB and native attempts, and never restores an old DB snapshot while newer workers may be live. If old binary cannot read migrated state, roll forward or provide a compatibility reader; no destructive downgrade.
