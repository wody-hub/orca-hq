...
async function runtimeEnvironment(): Promise<NodeJS.ProcessEnv> {
  ...
  try {
    await symlink(sourceAuth, authPath);
  } catch (error) {
    if (
      (error as NodeJS.ErrnoException).code !== "EEXIST" ||
      (await readlink(authPath).catch(() => "")) !== sourceAuth
    ) {
...
export function createCodexSessionClient(options: CodexSessionOptions) {
  ...
  async function execute(
    input: CodexSessionRun,
    register: (stop: () => void) => void,
  ): Promise<{ threadId: string; text: string }> {
    if (input.signal?.aborted) throw new Error("Codex 요청이 중지되었습니다.");
    if (Buffer.byteLength(input.text) > MAX_BYTES)
      throw new Error("Codex 입력 크기 제한을 초과했습니다.");
    ...
    try {
      child = (options.spawnProcess ?? spawn)(
        ...
        argumentsForServer(),
        { cwd: options.cwd, env, stdio: "pipe" },
      ) as ChildProcessWithoutNullStreams;
    } catch {
      throw new Error("Codex 프로세스를 시작하지 못했습니다.");
    }
    ...
    function stop(error?: Error) {
      ...
      for (const request of pending.values()) {
        ...
        request.reject(error ?? new Error("Codex 연결이 종료되었습니다."));
      }
      pending.clear();
    ...
    const abort = () => stop(new Error("Codex 요청이 중지되었습니다."));
    ...
    const deadline = setTimeout(
      () =>
        stop(
          new Error(
            "Codex 응답 제한 시간이 지났습니다. 이미 요청한 작업은 자동 재실행하지 않습니다.",
          ),
        ),
    ...
    function send(message: unknown) {
      ...
      try {
        child.stdin.write(JSON.stringify(message) + "\n", (error) => {
          if (error) stop(new Error("Codex 연결에 쓰지 못했습니다."));
        });
      } catch {
        stop(new Error("Codex 연결에 쓰지 못했습니다."));
      }
    }
    function request(method: string, params: unknown): Promise<RecordValue> {
      if (stopped)
        return Promise.reject(
          failure ?? new Error("Codex 연결이 종료되었습니다."),
        );
      const id = ++sequence;
      return new Promise((resolve, reject) => {
        const timer = setTimeout(
          () =>
            stop(
              new Error(
                `Codex ${method === "thread/resume" ? "대화 재개" : "요청"} 제한 시간이 지났습니다.`,
              ),
            ),
          Math.min(options.timeoutMs ?? 180_000, 30_000),
        );
    ...
    function assistantItem(item: RecordValue) {
      if (
        ...
        if (typeof item.id === "string") {
          if (commentaryItems.has(item.id)) return;
          commentaryItems.add(item.id);
        }
        const progress = Promise.resolve().then(() =>
          stopped
            ? undefined
            : input.onProgress!(redactRelayText(item.text).slice(0, 2000)),
        );
        inflight.add(progress);
        void progress.finally(() => inflight.delete(progress)).catch(() => {});
      }
      if (
        item.type === "agentMessage" &&
        item.phase !== "commentary" &&
    ...
    function handle(message: RecordValue) {
          ...
          if (message.error)
            req.reject(
              new Error(
                "Codex 요청 실패: " +
                  redactRelayText(
                    String(record(message.error).message ?? "원인 미확인"),
                  ).slice(0, 1000),
              ),
            );
          else req.resolve(record(message.result));
        }
        return;
      }
      ...
      if (message.id !== undefined) {
        ...
        if (
          ...
          send({
            ...
            result: {
              success: false,
              contentItems: [
                { type: "inputText", text: "허용하지 않는 도구 요청입니다." },
              ],
            },
          });
          return;
          ...
          work = Promise.resolve().then(async () => {
            if (stopped)
              return {
                success: false,
                contentItems: [
                  { type: "inputText", text: "요청 시간이 종료되었습니다." },
                ],
              };
            try {
              ...
              if (Buffer.byteLength(text) > MAX_BYTES)
                throw new Error("도구 결과 크기 제한 초과");
              return {
                success: true,
              ...
              return {
                ...
                contentItems: [
                  {
                    type: "inputText",
                    text: `HQ 도구 실행 실패: ${redactRelayText(error instanceof Error ? error.message : "알 수 없는 오류").slice(0, 2000)}. 동일 작업을 자동 재실행하지 마세요.`,
                  },
                ],
              };
            }
          });
          calls.set(params.callId, work);
          inflight.add(work);
          const tracked = work;
        ...
        void work
          ...
          .catch(() => stop(new Error("Codex 도구 결과 전달에 실패했습니다.")));
        return;
      }
      if (params.threadId !== threadId || !turnSubmitted) return;
      if (params.turnId && turnId && params.turnId !== turnId) return;
      if (message.method === "item/completed")
        assistantItem(record(params.item));
      if (message.method === "turn/completed") {
        const turn = record(params.turn);
        if (turnId && turn.id !== turnId) return;
        if (Array.isArray(turn.items))
          for (const item of turn.items) assistantItem(record(item));
        if (turn.status !== "completed")
          stop(
            new Error(
              "Codex 응답 중단: " +
                redactRelayText(
                  String(record(turn.error).message ?? turn.status),
                ).slice(0, 1000),
            ),
          );
        else complete();
    ...
    child.stdout.on("data", (chunk: string) => {
      ...
      if (
        bytes > 8 * MAX_BYTES ||
        Buffer.byteLength(buffer) + Buffer.byteLength(chunk) >= 2 * MAX_BYTES
      ) {
        stop(new Error("Codex 출력 크기 제한을 초과했습니다."));
        return;
      }
      buffer += chunk;
      for (;;) {
        ...
        try {
          ...
          stop(new Error("Codex 응답 형식을 해석하지 못했습니다."));
        }
      }
    });
    ...
    for (const stream of [child.stdin, child.stdout, child.stderr])
      stream.on("error", () =>
        stop(new Error("Codex 연결 오류가 발생했습니다.")),
      );
    child.on("error", () =>
      stop(new Error("Codex 프로세스를 시작하지 못했습니다.")),
    );
    child.on("close", () => {
      processClosed = true;
      markClosed();
      stop(new Error("Codex 프로세스가 응답 전에 종료되었습니다."));
    });
    try {
      await request("initialize", {
        clientInfo: { name: "orca_hq", version: "0.1.0" },
      ...
      try {
        ...
        if (input.threadId && !failure)
          throw new Error(
            "Codex 대화를 재개하지 못했습니다. /new로 새 대화를 시작할 수 있습니다.",
          );
        throw error;
      }
      ...
      if (
        ...
        throw new Error("Codex 대화 식별자가 올바르지 않습니다.");
      ...
      if (typeof turnId !== "string" || !turnId)
        throw new Error("Codex 응답 식별자가 올바르지 않습니다.");
      await completion;
      if (failure) throw failure;
      if (!finalText.trim())
        throw new Error("Codex가 최종 응답을 반환하지 않았습니다.");
  ...
  return {
    run(input: CodexSessionRun): Promise<{ threadId: string; text: string }> {
      if (closed)
        return Promise.reject(new Error("Codex 대화 연결이 종료되었습니다."));
      const entry = {
        stop: () => {},
        done: Promise.resolve() as Promise<unknown>,
      };
      const done = execute(input, (stop) => {
        entry.stop = stop;
      });
      entry.done = done;
      active.add(entry);
