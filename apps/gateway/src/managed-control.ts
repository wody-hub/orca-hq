...
export async function ensureOwnerOnlyDirectory(path: string): Promise<void> {
  ...
  if (
    ...
    throw new Error("control_directory_unavailable");
...
async function removeStale(path: string): Promise<void> {
  try {
    ...
    if (!file.isSocket() || file.uid !== process.getuid?.())
      throw new Error("control_path_unavailable");
  } catch (e) {
  ...
  const stale = await new Promise<boolean>((resolve, reject) => {
    ...
    socket.once("timeout", () => {
      socket.destroy();
      reject(new Error("control_socket_busy"));
    });
    socket.once("error", (e: NodeJS.ErrnoException) => {
      socket.destroy();
      if (e.code === "ECONNREFUSED" || e.code === "ENOENT") resolve(true);
      else reject(new Error("control_socket_unavailable"));
    });
  });
  if (!stale) throw new Error("control_socket_busy");
...
export async function startManagedControl(options: {
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      const result = await options.execute(
        sessionId === undefined ? command : { ...command, sessionId },
      );
      res.writeHead(200);
      ...
      res.end(
        JSON.stringify({
          text: "명령을 처리하지 못했습니다. 입력과 HQ 로그를 확인하세요.",
        }),
      );
    }
  });
  ...
  try {
    ...
    await unlink(options.socketPath).catch((e) => {
      if ((e as NodeJS.ErrnoException).code !== "ENOENT") throw e;
    });
    throw error;
  }
