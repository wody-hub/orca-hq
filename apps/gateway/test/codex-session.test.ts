...
const tools = [
  { name: "lookup", description: "조회", inputSchema: { type: "object" } },
];
...
describe("Codex persistent app-server client", () => {
  let fixture: string;
  beforeEach(async () => {
    fixture = await mkdtemp(join(tmpdir(), "hq-codex-session-test-"));
  ...
  it("persists the thread before submitting text and resumes it in another process", async () => {
    ...
    const onThread = vi.fn(async (id: string) => {
      ...
      expect(
        fake.received.filter((m) => m.method === "turn/start"),
      ).toHaveLength(0);
    });
    expect(await client.run({ ...input, onThread })).toEqual({
      threadId: "thread-1",
      text: "최종 답변",
    });
    await client.run({ ...input, threadId: "thread-1" });
    expect(
      fake.received.filter((m) => m.method === "thread/start"),
    ).toHaveLength(1);
    expect(
      fake.received.find((m) => m.method === "thread/start")?.params
        .dynamicTools[0].type,
    ).toBe("function");
    expect(
      fake.received.find((m) => m.method === "thread/resume")?.params.threadId,
    ).toBe("thread-1");
    expect(fake.spawnMock.mock.calls[0]?.[2]?.env?.CODEX_HOME).toBe(
      fake.spawnMock.mock.calls[1]?.[2]?.env?.CODEX_HOME,
    );
    await client.close();
  });
  it("returns repeated dynamic tool results and failures, rejecting unknown requests", async () => {
    ...
    const fake = server((m, send) => {
      ...
      if (m.id === 101) {
        ...
        expect(m.result.contentItems[0].text).toContain("도구 실패");
        expect(m.result.contentItems[0].text).not.toContain(
          "xoxb-secret-value",
        );
        send({
          id: 102,
        ...
        send({
          ...
          params: {
            ...
            turn: {
              ...
              status: "completed",
              items: [
                { type: "agentMessage", phase: "final_answer", text: "완료" },
              ],
            },
          },
        });
    ...
    const onTool = vi.fn(async (_name, args, _callId) => {
      if (args.count === 1) throw new Error("도구 실패 xoxb-secret-value");
      return { found: true };
    });
    const client = createCodexSessionClient({
      ...options,
    ...
    expect((await client.run({ ...input, onTool })).text).toBe("완료");
  ...
  it("does not settle a timeout until its active mutation settles or execute later tools", async () => {
    ...
    let settled = false;
    const result = client.run({ ...input, onTool }).then(
      () => {
        settled = true;
      },
      (error) => {
    ...
    release();
    expect((await result).message).toContain("시간");
    expect(onTool).toHaveBeenCalledTimes(1);
  });
  it.each(["exit", "overflow", "resume"])(
    "reports %s without replaying turns",
    async (mode) => {
      const fake = server((m, send, child) => {
        if (mode === "resume" && m.method === "thread/resume") {
          send({ id: m.id, error: { message: "private detail" } });
          return true;
        }
        if (m.method === "turn/start") {
          if (mode === "exit") child.emit("close", 1);
          else child.stdout.write("x".repeat(2 * 1024 * 1024));
          return true;
        }
      });
      const client = createCodexSessionClient({
      ...
      await expect(
        client.run({
          ...input,
          ...(mode === "resume" ? { threadId: "thread-1" } : {}),
        }),
      ).rejects.toThrow(/Codex/);
      expect(
        fake.received.filter((m) => m.method === "turn/start").length,
  ...
  it("isolates credentials and disables built-in execution while retaining durable history", async () => {
    ...
    const start = fake.received.find(
      (m) => m.method === "thread/start",
    )?.params;
    expect(start.sandbox).toBe("read-only");
    expect(start.ephemeral).toBe(false);
    expect(start.baseInstructions).toBe(options.instructions);
    await client.close();
    await expect(client.run(input)).rejects.toThrow("종료");
  });

  it("escalates termination if the app-server ignores SIGTERM", async () => {
    const fake = server((_m, _send, child) => {
      child.kill = vi.fn((signal) => {
        if (signal === "SIGKILL") queueMicrotask(() => child.emit("close", 0));
        return true;
      });
    });
    const client = createCodexSessionClient({
  ...
  it("close waits for an active mutation and prevents queued callbacks", async () => {
    ...
    release();
    await closing;
    expect((await run).message).toContain("종료");
    expect(onTool).toHaveBeenCalledTimes(1);
  });

  it("cancels a resource wait on turn timeout without launching a mutation", async () => {
    let aborted = false;
    const fake = server((m, send) => {
      if (m.method !== "turn/start") return;
      send({ id: m.id, result: { turn: { id: "turn-1" } } });
    ...
    await expect(
      client.run({
        ...input,
        onTool: async (_name, _args, _callId, signal) =>
  ...
  it("deduplicates the same commentary item replay but preserves distinct identical messages", async () => {
    const fake = server((m, send) => {
      if (m.method !== "turn/start") return;
      send({ id: m.id, result: { turn: { id: "turn-1" } } });
      const first = {
      ...
      const second = { ...first, id: "comment2" };
    ...
    await client.run({ ...input, onProgress });
